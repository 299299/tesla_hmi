...
JNIEXPORT void JNICALL SDL_JAVA_INTERFACE(onNativeMessage)(
        JNIEnv* env, jclass jcls,
        jint msg_code, jdouble data1, jdouble data2, jdouble data3, jstring data4);
...
static jmethodID midSendMessage2;
...
JNIEXPORT void JNICALL SDL_JAVA_INTERFACE(nativeSetupJNI)(JNIEnv* mEnv, jclass cls)
{
    ...
    midSendMessage2 = (*mEnv)->GetStaticMethodID(mEnv, mActivityClass,
                                "sendMessage2", "(IDDDDD)V");

    if (!midGetNativeSurface ||
       !midSetActivityTitle || !midSetOrientation || !midGetContext || !midInputGetInputDeviceIds ||
       !midSendMessage || !midShowTextInput || !midIsScreenKeyboardShown ||
       !midClipboardSetText || !midClipboardGetText || !midClipboardHasText || !midSendMessage2 ||
...
void Android_Callback(int, double, double, double, const char*);
JNIEXPORT void JNICALL SDL_JAVA_INTERFACE(onNativeMessage)(
                                    JNIEnv* env, jclass jcls,
                                    jint msg_code,
                                    jdouble data1,
                                    ...
                                    jstring data4)
{
    const char *utfname = (*env)->GetStringUTFChars(env, data4, NULL);
    Android_Callback(msg_code, data1, data2, data3, utfname);
    (*env)->ReleaseStringUTFChars(env, data4, utfname);
}
...
/* sends message to be handled on the UI event dispatch thread */
int Android_JNI_SendMessage2(int cmd, double data1, double data2, double data3, double data4, double data5)
{
    JNIEnv *env = Android_JNI_GetEnv();
    jboolean success;
    (*env)->CallStaticVoidMethod(env, mActivityClass, midSendMessage2, cmd, data1, data2, data3, data4, data5);
    return 1;
}
